...
subext='.nh.REF-SUB.fits'
for i in xrange(0,len(bands)):
    filename_format = 'N300-1.Q2.'+bands[i]+'.*.*.*.005606N3754.0060'+subext
    filename_done='N300-1.Q2.'+bands[i]+'.*.*.*.005606N3754.0060'+outext
    ...
    for j in xrange(0,len(files)):
        #print j
        file_original=files[j].replace(subext, '.nh.REF-SUBconv.fits')
...
#            i] + '\' -p 92.654754:-34.14111 -psf 2 -fwhm 5 -v -n 3.0  -y 2016 -s 14.0 -r 1000 -f 19 --fit_sky ' + file_original + ' '+'-d'+' '+files[j]+ ' ' + file_phot + ' > ' + file_output
        run_command = 'python ~/SNAP3/MagCalc.py -c  aavso -o  N300-1.Q2 -b \'' + bands[
            i] + '\' -p 13.67719:-37.62019 -psf 2 -fwhm 5 -v -n 3.0  -y 2016 -s 14.0 -r 1000 -f 19 --fit_sky ' + file_original + ' ' +'-d'+' '+files[j]+ ' '+ file_phot + ' > ' + file_output
        print run_command
