...
for i,sn_name in enumerate(file_names):
    ...
    print magB[:, 0]
    if sn_name=='KSPN2188_v1':
        ax1.plot(magB[:, 0] - magB[0, 0], magB[:, 3], linestyle='', markersize=11, marker=(5, 1), color='red', label='KSP-SN-2016kf')
    ...
    #     ax1.plot(magB[:, 0] , -1*magB[:, 1], linestyle='', marker=mark, color=col,label=sn_name)
        ...
        ax1.plot(magB[:,0]-magB[0,0],magB[:,3], linestyle = '-', markersize=3, marker=mark,color=col,label=sn_name)
    ax1.set_xlim([-30, 250])
    ...
    if sn_name=='SN2014cx':
        ...
        cx_m=magV[:,1]
    if sn_name=='KSPN2188_v1':
        ax2.plot(magV[:,0]-magV[0,0],magV[:,3], linestyle = '', markersize=11, marker=(5, 1), color='red',label='KSP-SN-2016kf')
    # elif sn_name == 'SN1999em':
    #     magV= np.genfromtxt('1999em_V.csv', delimiter=",")
    #     ax2.plot(magV[:, 0], -1 * magV[:, 1], linestyle='', marker=mark, color=col, label=sn_name)
    else:
        ax2.plot(magV[:,0]-magV[0,0],magV[:,3], linestyle = '-', markersize=3, marker=mark,color=col,label=sn_name)
    ax2.set_xlim([-30, 250])
    ax2.set_ylim([-18, -13])
    ...
    if magI.size>0:
        if sn_name == 'KSPN2188_v1':
            ...
            ax3.plot(tt,mag_abs, linestyle = '', markersize=11,marker=(5, 1), color='red',label='KSP-SN-2016kf')
        # elif sn_name == 'SN1999em':
        #     magI = np.genfromtxt('1999em_I.csv', delimiter=",")
        #     ax3.plot(magI[:, 0], -1 * magI[:, 1], linestyle='', marker=mark, color=col, label=sn_name)
        else:
            ax3.plot(magI[:, 0] - magI[0, 0], magI[:, 3], linestyle='-', markersize=3, marker=mark, color=col, label=sn_name)
        ax3.set_xlim([-30, 250])
        ax3.set_ylim([-18, -14])

plt.subplot(221)
label=ax1.set_ylabel('Absolute Magnitude')
ax1.yaxis.set_label_coords(-0.19, 0)
ax1.grid()
