...
ax = plt.subplot(211)
plt.plot(spec_SN[:,0],spec_SN[:,1]*(182**2)-3000,color='red')
print SN1999em[:,0]
SN2013fs_z=	0.011855
SN1999em_z=0.002392
SN2009ib_z=0.00482
...
print np.nanmax(SN1999em[:,1]),np.nanmax(SN2009ib[:,1]),np.nanmax(SN2012aw[:,1]),np.nanmax(spec_SN[:,1])
plt.plot(SN1999em[:,0],(SN1999em[:,1]*(d_99em**2))+550,color='black',label='SN1999em')
plt.plot(SN2009ib[:,0],SN2009ib[:,1]*(d_09ib**2)/1e-15 +1500,color='black',label='SN2009ib')
#plt.plot(SN2013fs[:,0],SN2013fs[:,1]/np.nanmax(SN2013fs[:,1])+2,color='black',label='SN2013fs')
plt.plot(SN2012aw[:,0],SN2012aw[:,1]*(d_12aw**2)/1e-15 +6000,color='black',label='SN2012aw')
...
label=ax.set_ylabel(r'Scaled Flux Density ($f_\lambda$) + Constant',fontsize=14)
ax.yaxis.set_label_coords(-0.02, 0)
...
ax.text(6800,-300, 'KSP-OT-2016kf (+96)', fontsize=14,multialignment='center',va='center')
ax.text(7050,2100, 'SN1999em (+94)', fontsize=14,multialignment='center',va='center')
ax.text(7050,5300, 'SN2009ib (+103)', fontsize=14,multialignment='center',va='center')
...
for l in my_list:
    ...
    ax.text(float(l[0])-45,-3500, l[1], fontsize=11,rotation=90,multialignment='center',va='center')
...
for l in my_list:
    ...
    if l[1]=='[NII]':
        ...
        ax2.axvline(x=float(l[0]),ls='--', ymax=1, color='black',lw=1)
        ax2.text(6480,0, l[1], fontsize=12,rotation=90,multialignment='center',va='center')
    elif float(l[0]) == 4959:
        ax2.axvline(x=float(l[0]), ls='--', ymax=1, color='black', lw=1)
        ax2.text(4959-80, 0.7, l[1], fontsize=12, rotation=90, multialignment='center', va='center')
    elif float(l[0]) == 4861.32:
        ax2.axvline(x=float(l[0]), ls='--', ymax=1, color='black', lw=1)
        ax2.text(float(l[0]),0, l[1], fontsize=12,rotation=90,multialignment='center',va='center')
    else:
        ax2.axvline(x=float(l[0]),ls='--', ymax=1, color='black',lw=1)
...
plt.savefig('/home/afsari/PycharmProjects/kspSN/plots/p_spectra.pdf')
plt.show()
